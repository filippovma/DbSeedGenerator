...
namespace ObjectDecomposer
{
    public static class ObjectDecomposer
    {
        public static TreeNode Decompose(object o)
        {
            ...
            properties = properties.Where(p => !(p.PropertyType.IsPrimitive || p.PropertyType.IsValueType || p.PropertyType == typeof(string) || p.PropertyType.ImplementsInterface(typeof(IList<>)) || p.PropertyType.ExtendsClass(typeof(List<>)))).ToArray();
